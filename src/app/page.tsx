...
type ShoppingItem = { id: string; text: string; checked: boolean; category?: string };
...
const DetailContent = ({
  ...
  useEffect(() => {
    if (!familyId) return;
    const docRef = doc(db, "users", familyId, "calendar", dateKey);
    const unsubscribe = onSnapshot(docRef, (docSnap) => {
      if (docSnap.exists()) {
        const d = docSnap.data();
        setData({ meals: d.meals || [], shopping: d.shopping || [] });
      } else {
  ...
  const saveToFirestore = async (newData: DayData) => {
    if (!familyId) return;
    await setDoc(doc(db, "users", familyId, "calendar", dateKey), newData, { merge: true });
  };
    ...
    <div className="h-full flex flex-col text-white">
      {isMobile ? (
        <div className="flex items-center justify-between p-4 border-b border-white/10">
          ...
          <button
            onClick={onClose}
            className="p-2 mt-2 bg-white/10 rounded-full hover:bg-white/20 transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
...
const CalendarCell = ({
  ...
  const handleDrop = async (e: React.DragEvent) => {
    ...
    try {
      ...
      const { sourceDateKey, type, item } = JSON.parse(transferData);
  ...
  return (
    <div
      ...
      className={clsx(
        ...
        isSelected
          ? "bg-emerald-500/40 border-emerald-400 shadow-lg ring-1 ring-emerald-400 scale-[1.02]"
    ...
    >
      <span className="text-sm lg:text-lg font-bold drop-shadow-md mb-1.5 text-white pointer-events-none">
        {day}
      </span>
      <div className="flex flex-col gap-1 w-full flex-1 overflow-hidden">
        {firstMeal && (
          <div
            ...
            className="cursor-grab active:cursor-grabbing w-full text-[10px] lg:text-xs font-bold px-1.5 py-1 rounded bg-emerald-500/30 text-emerald-50 border border-emerald-400/20 truncate shadow-sm flex items-center hover:bg-emerald-500/50 transition-colors"
          >
        ...
        {firstShopping && (
          <div
            ...
            className="cursor-grab active:cursor-grabbing w-full text-[10px] lg:text-xs font-bold px-1.5 py-1 rounded bg-cyan-500/30 text-cyan-50 border border-cyan-400/20 truncate shadow-sm flex items-center hover:bg-cyan-500/50 transition-colors"
          >
...
export default function Home() {
  ...
  const changeMonth = (diff: number) => {
    setDirection(diff);
    setCurrentDate(new Date(currentYear, currentMonth - 1 + diff, 1));
    setSelectedDay(null);
  };
    ...
    <div className="relative min-h-screen overflow-hidden text-white flex">
      ...
      <div className="fixed inset-0 -z-10 bg-black">
        ...
        <motion.div
          ...
          className="absolute w-[80vw] h-[80vh] rounded-full bg-emerald-600/20 blur-[130px]"
        />
        <motion.div
          animate={{
            x: ["0vw", "-25vw", "15vw", "0vw"],
          ...
          <>
            ...
            <AnimatePresence>
              ...
              <motion.div
                ...
                className="lg:hidden fixed inset-x-0 bottom-0 z-50 h-[75vh] rounded-t-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col border-t border-white/10 bg-black/40 backdrop-blur-2xl"
              >
                <DetailContent
                  dateKey={selectedDateKey}
                  displayDate={displayDateStr}
