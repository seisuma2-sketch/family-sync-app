          ...
          <div className="space-y-2">
            <AnimatePresence>
              {data.shopping.map((item) => (
                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9 }} key={item.id} onClick={() => toggleShoppingCheck(item.id)} 
                  className={clsx("flex items-center justify-between p-3 rounded-xl cursor-pointer transition-all border shadow-lg backdrop-blur-md group", item.checked ? "bg-black/30 border-transparent opacity-50" : "bg-white/10 border-white/10 hover:border-emerald-400/50 hover:bg-white/20")}
                >
                  <div className="flex items-center space-x-3 overflow-hidden flex-1">
                    {item.checked ? <CheckCircle2 className="w-5 h-5 text-emerald-500 shrink-0" /> : <Circle className="w-5 h-5 text-white/30 shrink-0" />}
                    <span className={clsx("text-sm font-bold transition-all truncate", item.checked ? "text-white/40 line-through font-medium" : "text-white")}>{item.text}</span>
                  </div>
                  <button onClick={(e) => { e.stopPropagation(); handleDeleteShopping(item.id); }} className="p-1.5 text-white/30 hover:text-red-400 hover:bg-red-400/10 rounded-lg transition-all shrink-0 ml-2 md:opacity-0 group-hover:opacity-100">
                    <Trash2 className="w-4 h-4" />
                  </button>
                </motion.div>
              ))}
  ...
  return (
    <div className="relative min-h-screen overflow-hidden text-white flex">
      ...
      <div className="relative z-10 flex items-start flex-1 min-h-screen">
        <div className="flex-1 min-w-0 p-4 lg:p-10 pb-24 lg:pb-8 flex flex-col h-screen">
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center space-x-4">
              <h2 className="text-2xl lg:text-3xl font-extrabold text-white drop-shadow-md tracking-tight">{currentYear}年 {currentMonth}月</h2>
              <div className="hidden md:flex space-x-1 ml-4">
                <button onClick={() => changeMonth(-1)} className="p-2 bg-white/10 backdrop-blur-md border border-white/10 rounded-xl hover:bg-white/20 shadow-sm transition-all"><ChevronLeft className="w-5 h-5" /></button>
                <button onClick={() => changeMonth(1)} className="p-2 bg-white/10 backdrop-blur-md border border-white/10 rounded-xl hover:bg-white/20 shadow-sm transition-all"><ChevronRight className="w-5 h-5" /></button>
              </div>
            </div>
            <button onClick={() => signOut(auth).then(() => router.push("/login"))} className="flex md:hidden items-center p-2.5 bg-black/30 backdrop-blur-md border border-white/10 rounded-xl text-white hover:bg-red-500/80 transition-all shadow-sm active:scale-95">
              <LogOut className="w-5 h-5" />
            </button>
          </div>

          <div className="flex md:hidden justify-end space-x-1 mb-4">
            <button onClick={() => changeMonth(-1)} className="p-2 bg-black/30 backdrop-blur-md border border-white/10 rounded-xl shadow-sm"><ChevronLeft className="w-5 h-5" /></button>
            <button onClick={() => changeMonth(1)} className="p-2 bg-black/30 backdrop-blur-md border border-white/10 rounded-xl shadow-sm"><ChevronRight className="w-5 h-5" /></button>
          </div>

          <div className="grid grid-cols-7 gap-1 lg:gap-4 mb-2 lg:mb-4">
            {["日", "月", "火", "水", "木", "金", "土"].map((day, i) => (
              <div key={day} className={clsx("text-center text-xs lg:text-sm font-bold drop-shadow-md", i === 0 ? "text-red-400" : i === 6 ? "text-blue-400" : "text-white/80")}>{day}</div>
            ))}
          </div>
          ...
          <div className="relative flex-1">
            <AnimatePresence initial={false} custom={direction}>
              <motion.div
                ...
                className="absolute inset-0 overflow-y-auto overflow-x-hidden pb-24 lg:pb-6 grid grid-cols-7 gap-1 lg:gap-3 grid-rows-[repeat(6,minmax(70px,1fr))] lg:grid-rows-[repeat(6,minmax(100px,1fr))] [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]"
              >
                {calendarDays.map((day, i) => (
                  <div key={i} className="w-full h-full">
                    {day && familyId ? (
                      <CalendarCell day={day} year={currentYear} month={currentMonth} familyId={familyId} isSelected={selectedDay === day} onClick={() => setSelectedDay(day)} />
